...
public:
    ...
    [[nodiscard]] BaseRegistrationResult
    align(const PointCloud::ConstPtr& observation,
          const Eigen::Isometry3d& initial_target_from_observation) const;

private:
    [[nodiscard]] Eigen::Isometry3d
    projectToAllowedDof(const Eigen::Isometry3d& target_from_observation,
                        const Eigen::Isometry3d& initial_target_from_observation) const;
    [[nodiscard]] bool
    timeBudgetExceeded(const std::chrono::steady_clock::time_point& started_at) const;
