...
RailModuleLocalizer::RailModuleLocalizer(ModuleLocalizationParameters parameters,
    ...
    if (!std::isfinite(parameters_.min_position_m) ||
        !std::isfinite(parameters_.max_position_m) ||
        ...
        !std::isfinite(parameters_.fine_half_window_m) ||
        parameters_.fine_half_window_m < 0.0 ||
        !std::isfinite(parameters_.max_correspondence_distance_m) ||
        parameters_.max_correspondence_distance_m <= 0.0 ||
        parameters_.min_correspondences == 0U ||
        !parameters_.roi_padding_m.allFinite() ||
        (parameters_.roi_padding_m.array() < 0.0).any()) {
...
ModuleLocalizationResult RailModuleLocalizer::locate(
    const PointCloud::ConstPtr& observation_in_base,
    const Eigen::Isometry3d& base_from_rail) const {
    ModuleLocalizationResult result;
    if (!observation_in_base || observation_in_base->empty()) {
        result.message = "module observation is empty";
        return result;
    }

    PointCloud::Ptr observation_in_rail(new PointCloud);
    pcl::transformPointCloud(*observation_in_base,
                             *observation_in_rail,
                             base_from_rail.inverse().matrix().cast<float>());

    PointCloud::Ptr roi(new PointCloud);
    const Eigen::Vector3d roi_min =
        model_min_ + Eigen::Vector3d(parameters_.min_position_m, 0.0, 0.0) -
        parameters_.roi_padding_m;
    const Eigen::Vector3d roi_max =
        model_max_ + Eigen::Vector3d(parameters_.max_position_m, 0.0, 0.0) +
        parameters_.roi_padding_m;
    roi->reserve(observation_in_rail->size());
    for (const auto& point : *observation_in_rail) {
        const Eigen::Vector3d value(point.x, point.y, point.z);
        if ((value.array() >= roi_min.array()).all() &&
            (value.array() <= roi_max.array()).all()) {
            roi->push_back(point);
        }
    }
    ...
    roi->is_dense = true;
    if (roi->empty()) {
        result.message = "module rail ROI contains no points";
        return result;
    }

    pcl::KdTreeFLANN<PointT> tree;
    tree.setInputCloud(roi);
    const double maximum_squared = parameters_.max_correspondence_distance_m *
                                   parameters_.max_correspondence_distance_m;
    ...
    const auto score = [&](const double position_m) {
        ...
        if (candidate.metrics.correspondence_count > 0U) {
            candidate.metrics.rmse_m = std::sqrt(
                squared_error_sum /
                static_cast<double>(candidate.metrics.correspondence_count));
            candidate.objective = candidate.metrics.rmse_m +
                                  parameters_.max_correspondence_distance_m *
                                      (1.0 - candidate.metrics.overlap_ratio);
        } else {
    ...
    const double fine_min = std::max(parameters_.min_position_m,
                                     best.position_m - parameters_.fine_half_window_m);
    const double fine_max = std::min(parameters_.max_position_m,
                                     best.position_m + parameters_.fine_half_window_m);
    for (double position = fine_min;
         position <= fine_max + 0.5 * parameters_.fine_step_m;
    ...
    result.metrics.search_boundary_hit =
        std::abs(result.position_m - parameters_.min_position_m) <=
            parameters_.fine_step_m ||
        std::abs(result.position_m - parameters_.max_position_m) <=
            parameters_.fine_step_m;
    std::string reason;
