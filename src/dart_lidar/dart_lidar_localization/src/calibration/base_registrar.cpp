...
#include "dart_lidar_localization/calibration/cloud_pyramid.hpp"
...
bool validNdtLevel(const NdtLevelParameters& level) {
    return std::isfinite(level.voxel_leaf_size_m) && level.voxel_leaf_size_m > 0.0 &&
           ...
           std::isfinite(level.transformation_epsilon) && level.transformation_epsilon > 0.0 &&
           level.max_iterations > 0;
}
...
bool validGicpLevel(const GicpLevelParameters& level) {
    return std::isfinite(level.voxel_leaf_size_m) && level.voxel_leaf_size_m > 0.0 &&
           ...
           std::isfinite(level.transformation_epsilon) && level.transformation_epsilon > 0.0 &&
           std::isfinite(level.fitness_epsilon) && level.fitness_epsilon > 0.0 &&
           level.max_iterations > 0;
}
...
BaseRegistrar::BaseRegistrar(BaseRegistrationParameters parameters, PointCloud::ConstPtr model)
    ...
    if (!std::isfinite(parameters_.max_total_time_ms) || parameters_.max_total_time_ms <= 0.0 ||
        parameters_.min_input_points == 0U) {
...
BaseRegistrar::align(const PointCloud::ConstPtr& observation,
    ...
    if (parameters_.coarse_enabled) {
        ...
        const auto sources = CloudPyramid::build(observation, ndtLeafSizes(parameters_.ndt_levels));
        for (std::size_t index = 0U; index < parameters_.ndt_levels.size(); ++index) {
            ...
            if (timeBudgetExceeded(started_at)) {
                return finish(BaseRegistrationStatus::kTimeBudgetExceeded,
                              "time budget exceeded after NDT level " + std::to_string(index));
            }
        }
    }
    ...
    for (std::size_t index = 0U; index < parameters_.gicp_levels.size(); ++index) {
        ...
        current = projectToAllowedDof(stage.target_from_source, initial_target_from_observation);
        if (timeBudgetExceeded(started_at)) {
            return finish(BaseRegistrationStatus::kTimeBudgetExceeded,
                          "time budget exceeded after GICP level " + std::to_string(index));
        }
    }
    ...
    return finish(BaseRegistrationStatus::kSuccess,
                  baseStatusMessage(BaseRegistrationStatus::kSuccess));
}

Eigen::Isometry3d
BaseRegistrar::projectToAllowedDof(const Eigen::Isometry3d& target_from_observation,
                                   const Eigen::Isometry3d& initial_target_from_observation) const {
    ...
    if (parameters_.dof_mode == DofMode::kXyYaw) {
        observation_from_model.translation().z() = initial_observation_from_model.translation().z();
    }
