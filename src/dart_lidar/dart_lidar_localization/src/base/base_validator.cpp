...
BaseRegistrationMetrics BaseValidator::evaluate(
    ...
    metrics.rmse_m = metrics.correspondence_count == 0U
                         ? std::numeric_limits<double>::infinity()
                         : std::sqrt(squared_error_sum /
                                     static_cast<double>(metrics.correspondence_count));
    ...
    const Eigen::Isometry3d initial_observation_from_model =
        initial_target_from_source.inverse();
    metrics.translation_from_initial_m =
        (observation_from_model.translation() - initial_observation_from_model.translation())
            .norm();
    metrics.yaw_from_initial_rad = std::abs(normalizedAngle(
        yawOf(observation_from_model) - yawOf(initial_observation_from_model)));
    metrics.search_boundary_hit = parameters.enforce_initial_deviation_limits &&
        (metrics.translation_from_initial_m >=
...
bool BaseValidator::accept(const BaseRegistrationMetrics& metrics,
    ...
    if (!std::isfinite(metrics.rmse_m) || metrics.rmse_m > parameters.max_rmse_m) {
        ...
        reason << "correspondences " << metrics.correspondence_count << " is below "
               ...
               metrics.translation_from_initial_m >
                   parameters.max_translation_from_initial_m) {
        reason << "translation correction " << metrics.translation_from_initial_m
               << "m exceeds " << parameters.max_translation_from_initial_m << "m";
    } else if (parameters.enforce_initial_deviation_limits &&
