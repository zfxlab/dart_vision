...
void emitLevelValues(YAML::Emitter& output,
                     const std::vector<LevelT>& levels,
                     AccessorT accessor) {
...
std::string ResultWriter::write(const std::string& directory,
    ...
    const auto output_path = chooseOutputPath(
        output_directory,
    ...
    output << YAML::Key << "rail" << YAML::Value << record.rail_frame;
    output << YAML::EndMap;
    output << YAML::Key << "models" << YAML::Value << YAML::BeginMap;
    output << YAML::Key << "base_path" << YAML::Value << record.base_model_path;
    output << YAML::Key << "module_path" << YAML::Value << record.module_model_path;
    output << YAML::EndMap;

    output << YAML::Key << "base" << YAML::Value << YAML::BeginMap;
    output << YAML::Key << "available" << YAML::Value << record.result.base.success();
    output << YAML::Key << "status_code" << YAML::Value
           << static_cast<int>(record.result.base.status);
    output << YAML::Key << "message" << YAML::Value << record.result.base.message;
    output << YAML::Key << "six_dof" << YAML::Value;
    emitTransform(output, record.result.reference_from_base);
    output << YAML::Key << "rmse_m" << YAML::Value << record.result.base.metrics.rmse_m;
    output << YAML::Key << "overlap_ratio" << YAML::Value
           << record.result.base.metrics.overlap_ratio;
    output << YAML::Key << "correspondence_count" << YAML::Value
           << record.result.base.metrics.correspondence_count;
    output << YAML::Key << "translation_from_initial_m" << YAML::Value
           << record.result.base.metrics.translation_from_initial_m;
    output << YAML::Key << "yaw_from_initial_rad" << YAML::Value
           << record.result.base.metrics.yaw_from_initial_rad;
    output << YAML::Key << "iterations" << YAML::Value
           << record.result.base.total_iterations;
    output << YAML::Key << "elapsed_ms" << YAML::Value
           << record.result.base.elapsed_ms;
    output << YAML::EndMap;

    output << YAML::Key << "module" << YAML::Value << YAML::BeginMap;
    output << YAML::Key << "enabled" << YAML::Value << record.module_parameters.enabled;
    output << YAML::Key << "available" << YAML::Value << record.result.module.available;
    output << YAML::Key << "message" << YAML::Value << record.result.module.message;
    output << YAML::Key << "position_m" << YAML::Value
           << record.result.module.position_m;
    output << YAML::Key << "rmse_m" << YAML::Value << record.result.module.metrics.rmse_m;
    output << YAML::Key << "overlap_ratio" << YAML::Value
           << record.result.module.metrics.overlap_ratio;
    output << YAML::Key << "correspondence_count" << YAML::Value
           << record.result.module.metrics.correspondence_count;
    output << YAML::EndMap;

    output << YAML::Key << "parameters" << YAML::Value << YAML::BeginMap;
    output << YAML::Key << "base" << YAML::Value << YAML::BeginMap;
    output << YAML::Key << "coarse_enabled" << YAML::Value
           << record.base_parameters.coarse_enabled;
    ...
    emitLevelValues(output, record.base_parameters.ndt_levels,
                    [](const auto& level) { return level.voxel_leaf_size_m; });
    output << YAML::Key << "ndt_resolution_m" << YAML::Value;
    emitLevelValues(output, record.base_parameters.ndt_levels,
                    [](const auto& level) { return level.resolution_m; });
    output << YAML::Key << "ndt_step_size_m" << YAML::Value;
    emitLevelValues(output, record.base_parameters.ndt_levels,
                    [](const auto& level) { return level.step_size_m; });
    output << YAML::Key << "ndt_transformation_epsilon" << YAML::Value;
    emitLevelValues(output, record.base_parameters.ndt_levels,
                    [](const auto& level) { return level.transformation_epsilon; });
    output << YAML::Key << "ndt_max_iterations" << YAML::Value;
    emitLevelValues(output, record.base_parameters.ndt_levels,
                    [](const auto& level) { return level.max_iterations; });
    output << YAML::Key << "gicp_voxel_leaf_size_m" << YAML::Value;
    emitLevelValues(output, record.base_parameters.gicp_levels,
                    [](const auto& level) { return level.voxel_leaf_size_m; });
    ...
    emitLevelValues(output, record.base_parameters.gicp_levels,
                    [](const auto& level) { return level.fitness_epsilon; });
    ...
    output << YAML::Key << "module" << YAML::Value << YAML::BeginMap;
    output << YAML::Key << "min_position_m" << YAML::Value
           << record.module_parameters.min_position_m;
    output << YAML::Key << "max_position_m" << YAML::Value
           << record.module_parameters.max_position_m;
    output << YAML::Key << "coarse_step_m" << YAML::Value
           << record.module_parameters.coarse_step_m;
    output << YAML::Key << "fine_step_m" << YAML::Value
           << record.module_parameters.fine_step_m;
    output << YAML::Key << "fine_half_window_m" << YAML::Value
           << record.module_parameters.fine_half_window_m;
    output << YAML::Key << "max_correspondence_distance_m" << YAML::Value
           << record.module_parameters.max_correspondence_distance_m;
    output << YAML::Key << "max_rmse_m" << YAML::Value
           << record.module_parameters.max_rmse_m;
    output << YAML::Key << "min_overlap_ratio" << YAML::Value
           << record.module_parameters.min_overlap_ratio;
    output << YAML::Key << "min_correspondences" << YAML::Value
           << record.module_parameters.min_correspondences;
    output << YAML::Key << "roi_padding_m" << YAML::Value;
    emitVector3(output, record.module_parameters.roi_padding_m);
    output << YAML::Key << "max_position_jump_m" << YAML::Value
           << record.module_parameters.max_position_jump_m;
    ...
    if (error) {
        ...
        throw std::runtime_error("failed to finalize localization result: " +
                                 error.message());
    }
