...
def generate_launch_description():
    ...
    default_preprocessing_params = PathJoinSubstitution(
        [bringup_share, "config", "lidar", "module", "preprocessing.yaml"]
    )
    default_accumulation_params = PathJoinSubstitution(
        [bringup_share, "config", "lidar", "module", "accumulation.yaml"]
    )
    default_localization_params = PathJoinSubstitution(
        [bringup_share, "config", "lidar", "module", "module_localization.yaml"]
    )
    ...
    localization = Node(
        ...
        executable="base_localization_node",
        name="base_localization",
        output="screen",
        condition=IfCondition(start_localization),
        parameters=[localization_params_file, common_time],
    )
